...
#  extract features that are either means or standard deviations     
   ...
   keep1 <- grepl("mean()", features, fixed = TRUE)
   keep2 <- grepl("std()", features, fixed = TRUE)
...
#  convert activity code to activity label   
   ...
   mdat$activity <- activities[mdat$activity, 2]
   
   
#  write activities as a text file
#  write data file as a text file.
   
   
   write.table(activities,"activities.txt", col.names = FALSE, row.names = FALSE)    
   write.table(mdat,"tidy_data.txt",  row.names = FALSE)
